...
final class ErrorTolerancePropertyTests: XCTestCase {
    ...
    func testProperty8_SkipUnsupportedBlockElements() throws {
        ...
        for _ in 0..<100 {
            // 随机选择一个不支持的标签
            let unsupportedTag = unsupportedTags.randomElement()!
            ...
            XCTAssertTrue(result.warnings.contains { $0.message.contains(unsupportedTag) },
                         "警告信息应该包含不支持的元素名称")
            
            // 验证内容
            guard let textBlock1 = document.blocks[0] as? TextBlockNode,
                  let textBlock2 = document.blocks[1] as? TextBlockNode else {
    ...
    func testProperty8_SkipUnsupportedInlineElements() throws {
        ...
        for _ in 0..<100 {
            ...
            let xml = """
            <text indent="1"><b>\(XMLEntityCodec.encode(validText1))</b><\(unsupportedTag)>\(XMLEntityCodec.encode(unsupportedContent))</\(unsupportedTag)><i>\(XMLEntityCodec.encode(validText2))</i></text>
            """
    ...
    func testProperty8_HandleMixedValidAndInvalidElements() throws {
        for _ in 0..<50 {
            ...
            // 验证：应该解析出所有有效的文本块
            XCTAssertEqual(document.blocks.count, expectedValidCount,
                          "应该解析出 \(expectedValidCount) 个有效的块级节点")
            
            // 验证警告数量（至少应该有实际插入的不支持元素数量的警告）
            XCTAssertGreaterThanOrEqual(result.warnings.count, actualUnsupportedCount,
            ...
            for (index, block) in document.blocks.enumerated() {
                ...
                XCTAssertEqual(extractPlainText(from: textBlock.content), validTexts[index],
                              "文本块 \(index) 的内容应该正确")
            }
        }
    }
    ...
    func testProperty8_HandleUnsupportedElementsInQuote() throws {
        for _ in 0..<50 {
            ...
            // 验证内容
            XCTAssertEqual(extractPlainText(from: quote.textBlocks[0].content), validText1,
                          "第一个文本块内容应该正确")
            XCTAssertEqual(extractPlainText(from: quote.textBlocks[1].content), validText2,
                          "第二个文本块内容应该正确")
        }
    }
    ...
    func testProperty8_FallbackToPlainTextOnInvalidXML() throws {
        ...
        for invalidXML in invalidXMLCases {
            ...
            if result.hasWarnings {
                // 有警告是好的，但不是必需的
                print("警告信息: \(result.warnings.map { $0.message })")
            }
        }
    }
    ...
    func testProperty8_ContinueParsingAfterError() throws {
        for _ in 0..<50 {
            ...
            for (index, block) in document.blocks.enumerated() {
                ...
                XCTAssertEqual(extractPlainText(from: textBlock.content), texts[index],
                              "文本块 \(index) 的内容应该正确")
            }
        }
    }
    ...
    func testProperty8_WarningTypesCorrectness() throws {
        ...
        XCTAssertTrue(result1.warnings.contains { $0.type == .unsupportedElement },
                     "应该有不支持元素类型的警告")
        ...
        XCTAssertTrue(result2.warnings.contains { $0.type == .unsupportedElement },
                     "应该有不支持元素类型的警告")
    }
