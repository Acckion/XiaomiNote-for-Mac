...
final class ChineseIMEEnterKeyFixTests: XCTestCase {
    ...
    func testEnterKeyInIMECompositionState_ShouldNotInsertNewline() async throws {
        // 这个测试主要验证逻辑正确性
        // 实际的输入法行为需要在真实环境中测试
    ...
    /// 期望：正常处理，不影响输入法状态
    func testOtherKeysInIMECompositionState() async throws {
    ...
    /// 场景：用户快速连续按两次回车
    /// 期望：第一次确认输入法，第二次换行
    func testRapidEnterKeyPresses() async throws {
        print("[测试] 快速连续按回车")
