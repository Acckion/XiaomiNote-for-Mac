...
final class ErrorHandlingPropertyTests: XCTestCase {
    ...
    /// 3. 验证错误被正确记录
    /// 4. 验证界面状态保持一致
    func testProperty8_FormatApplicationErrorRecovery() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            XCTAssertFalse(
                errorHistory.isEmpty,
                "迭代 \(iteration): 错误应该被记录到历史中"
            )

            // 8. 验证错误类型正确
            let lastError = try XCTUnwrap(errorHistory.last)
    ...
    /// **验证需求**: 4.1
    func testProperty8_EmptySelectionErrorHandling() throws {
        let iterations = 100
    ...
    /// **验证需求**: 4.1
    func testProperty8_ConsecutiveErrorDetection() {
        let iterations = 10
    ...
    /// 2. 验证错误被正确记录
    /// 3. 验证恢复操作是强制状态更新
    func testProperty9_StateSyncErrorRecovery() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            let reason = try XCTUnwrap(reasons.randomElement())
    ...
    /// **验证需求**: 4.2
    func testProperty9_StateInconsistencyDetection() {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            let expectedStr = expectedFormats.map(\.displayName).joined(separator: ", ")
            let actualStr = actualFormats.map(\.displayName).joined(separator: ", ")
    ...
    /// **验证需求**: 4.1, 4.2
    func testProperty9_ErrorStatisticsAccuracy() throws {
        let iterations = 50
        ...
        for iteration in 1 ... iterations {
            ...
            let error = try XCTUnwrap(errorTypes.randomElement())
