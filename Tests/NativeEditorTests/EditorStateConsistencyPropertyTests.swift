...
final class EditorStateConsistencyPropertyTests: XCTestCase {
    ...
    func testProperty10_EditorStateFormatButtonConsistency() async throws {
        let iterations = 100
    ...
    /// **验证需求**: 4.3
    func testProperty10_ReadOnlyStateDisablesFormatButtons() async throws {
        let iterations = 50
        ...
        for iteration in 1...iterations {
            ...
            // 2. 验证格式按钮被禁用
            XCTAssertFalse(stateChecker.formatButtonsEnabled,
    ...
    /// **验证需求**: 4.3
    func testProperty10_UnfocusedStateDisablesFormatButtons() async throws {
        let iterations = 50
    ...
    /// **验证需求**: 4.3
    func testProperty10_EditableStateEnablesFormatButtons() async throws {
        let iterations = 50
        ...
        for iteration in 1...iterations {
            ...
            // 2. 验证格式按钮被启用
            XCTAssertTrue(stateChecker.formatButtonsEnabled,
    ...
    /// **验证需求**: 4.3
    func testProperty10_StateTransitionConsistency() async throws {
        let iterations = 100
        ...
        for iteration in 1...iterations {
            ...
            print("[PropertyTest] 迭代 \(iteration): 状态序列=\(states.map { $0.description })")
    ...
    /// **验证需求**: 4.3
    func testProperty10_AllFormatTypesValidationConsistency() async throws {
        let iterations = 50
        ...
        for iteration in 1...iterations {
            ...
            for format in TextFormat.allCases {
                ...
                XCTAssertEqual(allowed, expectedAllowed,
    ...
    /// **属性**: 状态统计信息应该准确反映当前状态
    /// **验证需求**: 4.3
    func testProperty10_StateStatisticsAccuracy() async throws {
        let iterations = 50
        ...
        for iteration in 1...iterations {
            ...
            XCTAssertEqual(currentStateDesc, state.description,
                          "迭代 \(iteration): 统计信息中的当前状态应该准确")
            XCTAssertEqual(formatButtonsEnabled, state.allowsFormatting,
                          "迭代 \(iteration): 统计信息中的格式按钮启用状态应该准确")
            XCTAssertEqual(stateChangeReason, reason,
    ...
    private func generateRandomStateSequence(length: Int) -> [EditorState] {
        return (0..<length).map { _ in generateRandomEditorState() }
    }
}
