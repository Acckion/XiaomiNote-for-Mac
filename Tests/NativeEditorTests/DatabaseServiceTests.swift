...
final class DatabaseServiceTests: XCTestCase {
    ...
    ///
    /// **验证需求: 1.1-1.12**
    func testNotesTableHasAllRequiredFields() {
    ...
    func testMigrationIsIdempotent() {
        ...
        // 验证字段数量相同
        XCTAssertEqual(
            fieldsAfterFirstMigration.count,
            fieldsAfterSecondMigration.count,
            "重复迁移不应该改变字段数量"
        )

        // 验证字段内容相同
        XCTAssertEqual(
            Set(fieldsAfterFirstMigration),
            Set(fieldsAfterSecondMigration),
            "重复迁移不应该改变字段列表"
        )
