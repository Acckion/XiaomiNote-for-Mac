...
final class MixedFormatStatePropertyTests: XCTestCase {
    ...
    /// 3. 选中包含加粗和非加粗的范围
    /// 4. 验证混合格式状态被正确检测
    func testProperty14_PartialBoldMixedFormatStateDetection() {
        let iterations = 100
    ...
    /// **验证需求**: 6.2
    func testProperty14_MultipleMixedFormatStateDetection() {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            if let underlineState = states[.underline] {
                XCTAssertTrue(
                    underlineState.shouldShowAsActive,
    ...
    /// **验证需求**: 6.1
    func testProperty14_FullyActiveFormatStateDetection() {
        let iterations = 100
    ...
    /// **验证需求**: 6.1
    func testProperty14_InactiveFormatStateDetection() {
        let iterations = 100
    ...
    /// **验证需求**: 6.2
    func testProperty14_EditorContextMixedFormatStateIntegration() {
        let iterations = 50
        ...
        for iteration in 1 ... iterations {
            ...
            print("[PropertyTest]   - 部分激活格式: \(partiallyActive.map(\.displayName))")

            // 高亮格式应该在部分激活集合中
            XCTAssertTrue(
                partiallyActive.contains(.highlight),
