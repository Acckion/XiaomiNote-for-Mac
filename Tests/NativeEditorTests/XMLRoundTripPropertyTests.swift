...
final class XMLRoundTripPropertyTests: XCTestCase {
    ...
    func testProperty1_TextBlockRoundTrip() throws {
        for _ in 0 ..< 100 {
            ...
            // 验证语义等价
            XCTAssertTrue(
                areDocumentsSemanticEqual(document, document2),
                "往返后文档应该语义等价\n原始: \(originalXML)\n生成: \(generatedXML)"
            )
    ...
    func testProperty1_FormattedTextRoundTrip() throws {
        ...
        for _ in 0 ..< 100 {
            let tag = try XCTUnwrap(formatTags.randomElement())
            let textContent = generateRandomText()
            ...
            guard let textBlock = document2.blocks.first as? TextBlockNode,
                  let formattedNode = textBlock.content.first as? FormattedNode
            else {
                XCTFail("应该有格式化节点")
                continue
            }
    ...
    func testProperty1_HighlightTextRoundTrip() throws {
        for _ in 0 ..< 100 {
            ...
            guard let textBlock = document2.blocks.first as? TextBlockNode,
                  let highlightNode = textBlock.content.first as? FormattedNode
            else {
                XCTFail("应该有高亮节点")
                continue
            }
    ...
    func testProperty1_NestedFormatRoundTrip() throws {
        ...
        for _ in 0 ..< 100 {
            let (outerTag, innerTag) = try XCTUnwrap(formatPairs.randomElement())
            let textContent = generateRandomText()
