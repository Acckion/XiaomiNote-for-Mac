...
final class ParagraphManagerTests: XCTestCase {
    ...
    func testDetectParagraphBoundaries_CoverageAndNoOverlap() {
        ...
        for range in ranges {
            for location in range.location..<(range.location + range.length) {
                // 检查是否有重叠
                XCTAssertFalse(coveredLocations.contains(location), 
                             "位置 \(location) 被多个段落覆盖")
        ...
        // 检查是否覆盖了整个文本
        XCTAssertEqual(coveredLocations.count, text.count, 
                      "段落应该覆盖整个文本")
    }
    ...
    func testUpdateParagraphs_AdjustUnaffectedParagraphs() {
        ...
        XCTAssertEqual(thirdParagraphNewLocation, thirdParagraphInitialLocation + insertText.count, 
                      "第三段位置应该根据插入的文本长度调整")
    }
