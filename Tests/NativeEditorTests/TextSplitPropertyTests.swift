...
final class TextSplitPropertyTests: XCTestCase {
    ...
    func testPropertyTextSplitPreservesContent() {
        ...
        for iteration in 0 ..< 100 {
            // 随机选择文本或生成随机文本
            let text: String = if iteration < testTexts.count {
                testTexts[iteration]
            } else {
                generateRandomText(length: Int.random(in: 1 ... 50))
            }
    ...
    func testPropertyOrderedListSplitConsistency() {
        ...
        guard let bulletInfo = ListBehaviorHandler.getListItemInfo(in: bulletStorage, at: 0),
              let orderedInfo = ListBehaviorHandler.getListItemInfo(in: orderedStorage, at: 0)
        else {
            XCTFail("应该能获取列表项信息")
            return
        }
        ...
        guard let bulletSplit = ListBehaviorHandler.getTextSplitResult(in: bulletStorage, at: bulletMidPosition),
              let orderedSplit = ListBehaviorHandler.getTextSplitResult(in: orderedStorage, at: orderedMidPosition)
        else {
            XCTFail("应该能获取分割结果")
            return
        }
