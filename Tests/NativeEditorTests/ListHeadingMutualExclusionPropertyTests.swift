...
final class ListHeadingMutualExclusionPropertyTests: XCTestCase {
    ...
    /// 5. 验证列表格式已应用
    func testProperty5_1_ApplyBulletListRemovesHeadingFormat() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            // 3. 随机选择一个标题级别并应用
            let headingInfo = try XCTUnwrap(headingSizes.randomElement())
    ...
    /// **验证需求**: 5.1, 5.3
    func testProperty5_1_ApplyOrderedListRemovesHeadingFormat() throws {
        let iterations = 100
    ...
    /// 3. 调用标题-列表互斥处理
    /// 4. 验证列表格式已移除
    func testProperty5_2_ApplyHeadingRemovesListFormat() throws {
        let iterations = 100
    ...
    /// 2. 随机选择一个列表类型并应用
    /// 3. 验证字体大小为正文大小
    func testProperty5_3_ListAlwaysUsesBodyFontSize() throws {
        let iterations = 100
    ...
    /// 4. 应用第二种格式
    /// 5. 验证只有第二种格式存在
    func testProperty5_ListAndHeadingMutualExclusion() throws {
        let iterations = 100
    ...
    /// **验证需求**: 5.1, 5.3
    func testProperty5_PreserveBoldTraitWhenApplyingList() throws {
        let iterations = 100
