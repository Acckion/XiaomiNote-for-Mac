...
final class FormatApplicationPerformancePropertyTests: XCTestCase {
    ...
    /// 性能阈值（毫秒)
    let performanceThresholdMs = 50.0
    ...
    /// 3. 测量加粗格式应用时间
    /// 4. 验证应用时间在50ms内
    func testProperty5_BoldFormatApplicationPerformance() {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            XCTAssertLessThanOrEqual(
                durationMs,
                performanceThresholdMs * 2, // 允许一定的波动
    ...
    /// **属性**: 对于任何格式按钮点击操作，系统应该在50ms内开始应用格式
    /// **验证需求**: 3.1
    func testProperty5_ItalicFormatApplicationPerformance() {
        let iterations = 100
    ...
    /// **属性**: 对于任何格式按钮点击操作，系统应该在50ms内开始应用格式
    /// **验证需求**: 3.1
    func testProperty5_UnderlineFormatApplicationPerformance() {
        let iterations = 100
    ...
    /// 4. 测量格式应用时间
    /// 5. 验证应用时间在50ms内
    func testProperty5_AllInlineFormatsApplicationPerformance() throws {
        let iterations = 100
        ...
        var formatStats: [TextFormat: (total: Double, max: Double, sampleCount: Int, slow: Int)] = [:]
        ...
        for iteration in 1 ... iterations {
            ...
            // 3. 随机选择一种格式
            let format = try XCTUnwrap(inlineFormats.randomElement())
            ...
            // 更新统计
            var stats = try XCTUnwrap(formatStats[format])
        ...
        for (format, stats) in formatStats.sorted(by: { $0.key.displayName < $1.key.displayName }) {
            if stats.sampleCount > 0 {
                let avgDuration = stats.total / Double(stats.sampleCount)
                print(
                    "  - \(format.displayName): 平均 \(String(format: "%.2f", avgDuration))ms, 最大 \(String(format: "%.2f", stats.max))ms, 慢速 \(stats.slow)/\(stats.sampleCount)"
                )
    ...
    /// 3. 测量格式应用时间
    /// 4. 验证应用时间在50ms内
    func testProperty5_LargeTextFormatApplicationPerformance() {
