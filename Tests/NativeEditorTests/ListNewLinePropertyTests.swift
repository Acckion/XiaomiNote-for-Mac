...
final class ListNewLinePropertyTests: XCTestCase {
    ...
    /// 4. 验证新行继承了无序列表格式
    /// 5. 验证新行继承了缩进级别
    func testProperty6_BulletListNewLineInheritsFormat() async throws {
        let iterations = 100
    ...
    /// 4. 验证新行继承了有序列表格式
    /// 5. 验证新行编号正确递增
    func testProperty6_OrderedListNewLineInheritsFormatAndIncrementsNumber() async throws {
        let iterations = 100
    ...
    /// 4. 验证新行保留了列表格式
    /// 5. 验证新行清除了内联格式
    func testProperty6_ListNewLineClearsInlineFormats() async throws {
        let iterations = 100
        ...
        for iteration in 1...iterations {
            ...
            attributedString.addAttribute(.underlineStyle, value: NSUnderlineStyle.single.rawValue, range: NSRange(location: 0, length: testText.count))
            textStorage.setAttributedString(attributedString)
    ...
    /// 3. 验证列表格式被取消
    /// 4. 验证当前行恢复为普通正文格式
    func testProperty7_EmptyBulletListEnterCancelsFormat() async throws {
        let iterations = 100
    ...
    /// 3. 验证列表格式被取消
    /// 4. 验证当前行恢复为普通正文格式
    func testProperty7_EmptyOrderedListEnterCancelsFormat() async throws {
        let iterations = 100
    ...
    /// 3. 验证当前行恢复为普通正文格式
    /// 4. 验证字体大小为正文大小（14pt）
    func testProperty7_EmptyListEnterRestoresBodyFormat() async throws {
        let iterations = 100
