...
final class PerformanceMonitoringPropertyTests: XCTestCase {
    ...
    /// 3. 记录性能指标
    /// 4. 验证指标被正确记录
    func testProperty20_FormatApplicationMetricRecording() async throws {
        let iterations = 100
        ...
        for iteration in 1...iterations {
            ...
            XCTAssertGreaterThanOrEqual(records.count, iteration,
                "迭代 \(iteration): 格式应用指标应该被记录")
            
            // 4. 验证最后一条记录的内容
            if let lastRecord = records.last {
                XCTAssertEqual(lastRecord.format, format,
                    "迭代 \(iteration): 记录的格式应该匹配")
                XCTAssertEqual(lastRecord.success, success,
                    "迭代 \(iteration): 记录的成功状态应该匹配")
                XCTAssertEqual(lastRecord.duration, duration, accuracy: 0.0001,
        ...
        XCTAssertEqual(totalRecords.count, iterations,
    ...
    /// **验证需求**: 8.3
    func testProperty20_StateSynchronizationMetricRecording() async throws {
        let iterations = 100
    ...
    /// **验证需求**: 8.3
    func testProperty20_ThresholdExceededDetection() async throws {
        let iterations = 100
        ...
        XCTAssertEqual(exceededRecords.count, exceededCount,
    ...
    /// **验证需求**: 8.3
    func testProperty20_PerformanceStatisticsCalculation() async throws {
        let iterations = 100
        ...
        XCTAssertEqual(stats.averageDuration, expectedAvg, accuracy: 0.0001,
            "平均持续时间应该正确")
        
        // 验证最小值和最大值
        let sortedDurations = durations.sorted()
        XCTAssertEqual(stats.minDuration, sortedDurations.first!, accuracy: 0.0001,
            "最小持续时间应该正确")
        XCTAssertEqual(stats.maxDuration, sortedDurations.last!, accuracy: 0.0001,
            "最大持续时间应该正确")
    ...
    /// **验证需求**: 8.3
    func testProperty20_LogRecordingCompleteness() async throws {
        let iterations = 50
        ...
        XCTAssertGreaterThanOrEqual(performanceLogs.count, warningCount,
    ...
    /// **验证需求**: 8.3
    func testProperty20_PerformanceReportGeneration() async throws {
        let iterations = 100
        print("\n[PropertyTest] 开始属性测试: 性能报告生成 (迭代次数: \(iterations))")
        
        // 1. 生成随机性能记录
        for _ in 1...iterations {
            let type = FormatMenuMetricType.allCases.randomElement()!
        ...
        if !passed {
            ...
            for issue in issues {
                XCTAssertTrue(report.contains(issue) || report.contains("问题"),
                    "报告应包含问题信息")
            }
        }
    ...
    /// **验证需求**: 8.3
    func testProperty20_MultiTypeMetricRecording() async throws {
        let iterations = 100
        ...
        XCTAssertEqual(allRecords.count, iterations,
            "总记录数应该为 \(iterations)")
    ...
    /// **验证需求**: 8.3
    func testProperty20_RecordCleanup() async throws {
        let iterations = 50
        ...
        // 验证记录已生成
        XCTAssertEqual(performanceMonitor.getAllRecords().count, iterations,
            "应该有 \(iterations) 条记录")
        ...
        // 3. 测试全部清理
        performanceMonitor.clearAllRecords()
        XCTAssertEqual(performanceMonitor.getAllRecords().count, 0,
            "所有记录应该被清除")
