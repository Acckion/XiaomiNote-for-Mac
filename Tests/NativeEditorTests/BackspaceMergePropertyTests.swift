...
final class BackspaceMergePropertyTests: XCTestCase {
    ...
    /// 4. 验证内容合并到上一行
    /// 5. 验证合并后只有一行
    func testProperty5_MergeListItemToPreviousListItem() async throws {
        let iterations = 20
        ...
        for iteration in 1...iterations {
            ...
            XCTAssertTrue(mergedContent.contains(expectedContent), 
    ...
    /// 4. 验证内容合并到上一行
    /// 5. 验证列表格式被取消
    func testProperty5_MergeListItemToPlainText() async throws {
        let iterations = 20
    ...
    /// 3. 调用 handleBackspaceKey
    /// 4. 验证附件数量减少
    func testProperty5_BackspaceRemovesListMarker() async throws {
        let iterations = 20
    ...
    /// 3. 调用 handleBackspaceKey
    /// 4. 验证光标位置在合并点
    func testProperty5_CursorPositionAfterMerge() async throws {
        let iterations = 20
        ...
        for iteration in 1...iterations {
            ...
            XCTAssertEqual(actualCursorPosition, expectedCursorPosition, 
    ...
    /// 3. 调用 handleBackspaceKey
    /// 4. 验证不触发合并（返回 false）
    func testProperty5_NoMergeWhenNotAtContentStart() async throws {
        let iterations = 20
    ...
    /// 3. 调用 handleBackspaceKey
    /// 4. 验证不触发合并（返回 false）
    func testProperty5_NoMergeOnFirstLine() async throws {
        let iterations = 20
