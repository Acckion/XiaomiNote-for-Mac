...
final class OrderedListNumberingPropertyTests: XCTestCase {
    ...
    /// 2. 应用有序列表格式
    /// 3. 验证编号为 1
    func testProperty7_NewOrderedListStartsFromOne() {
        let iterations = 20
    ...
    /// 2. 在列表中间插入新项
    /// 3. 验证所有编号连续递增
    func testProperty7_InsertionMaintainsConsecutiveNumbering() {
        let iterations = 20
        ...
        for iteration in 1 ... iterations {
            ...
            while position < textStorage.length {
                ...
                ListFormatHandler.applyOrderedList(
                    to: textStorage,
            ...
            if let info = listInfo {
                ...
                _ = ListBehaviorHandler.splitTextAtCursor(
                    textView: textView,
    ...
    /// 2. 删除列表中的某一项
    /// 3. 验证所有编号连续递增
    func testProperty7_DeletionMaintainsConsecutiveNumbering() {
        let iterations = 20
    ...
    /// 2. 调用重新编号方法
    /// 3. 验证编号从 1 开始连续递增
    func testProperty7_RenumberingEnsuresConsecutiveFromOne() {
        let iterations = 20
    ...
    /// 3. 重新编号后验证返回 true
    func testProperty7_ValidationDetectsNonConsecutiveNumbering() {
        let iterations = 20
        ...
        for iteration in 1 ... iterations {
            ...
            while position < textStorage.length, lineIndex < nonConsecutiveNumbers.count {
                ...
                ListFormatHandler.applyOrderedList(
                    to: textStorage,
                    range: NSRange(location: lineRange.location, length: 0),
                    number: nonConsecutiveNumbers[lineIndex],
                    indent: indent
                )
                position = lineRange.location + lineRange.length
                lineIndex += 1
