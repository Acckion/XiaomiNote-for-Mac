...
final class EditorIntegrationTests: XCTestCase {
    ...
    /// 测试编辑器工厂创建
    func testEditorFactoryCreation() {
        // 测试获取可用编辑器类型
    ...
    /// 测试简单文本的格式转换
    func testSimpleTextFormatConversion() throws {
    ...
    /// 测试复杂文档的格式转换
    func testComplexDocumentFormatConversion() throws {
        let complexXML = """
    ...
    /// 测试编辑器上下文的格式应用
    func testEditorContextFormatApplication() {
        // 应用加粗格式
    ...
    /// 测试编辑器上下文的内容加载
    func testEditorContextContentLoading() {
    ...
    /// 测试编辑器切换性能
    func testEditorSwitchPerformance() {
        measure {
            // 测试编辑器可用性检查性能（现在只有原生编辑器）
            _ = EditorFactory.getEditorInfo(for: .native)
        }
    }

    /// 测试格式转换性能
    func testFormatConversionPerformance() {
        let xml = "<text indent=\"1\">性能测试文本</text>"

        measure {
            do {
                let attributedString = try formatConverter.xmlToAttributedString(xml)
                _ = try formatConverter.attributedStringToXML(attributedString)
            } catch {
