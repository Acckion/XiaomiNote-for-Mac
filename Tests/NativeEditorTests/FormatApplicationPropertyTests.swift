...
final class FormatApplicationPropertyTests: XCTestCase {
    ...
    func testProperty1_BoldFormatApplicationConsistency() {
        let iterations = 100
    ...
    /// **验证需求**: 1.2
    func testProperty1_ItalicFormatApplicationConsistency() {
        let iterations = 100
    ...
    /// **验证需求**: 1.3
    func testProperty1_UnderlineFormatApplicationConsistency() {
        let iterations = 100
    ...
    /// **验证需求**: 1.4
    func testProperty1_StrikethroughFormatApplicationConsistency() {
        let iterations = 100
    ...
    /// **验证需求**: 1.5
    func testProperty1_HighlightFormatApplicationConsistency() {
        let iterations = 100
    ...
    /// 5. 再次应用相同标题格式（切换）
    /// 6. 验证标题格式已移除，恢复为正常文本
    func testProperty2_HeadingFormatApplicationCorrectness() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            // 3. 随机选择一个标题级别
            let headingInfo = try XCTUnwrap(headingLevels.randomElement())
    ...
    /// 3. 应用对齐格式（左对齐、居中、右对齐）
    /// 4. 验证选中段落的对齐方式正确
    func testProperty2_AlignmentFormatApplicationCorrectness() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            // 3. 随机选择一个对齐方式
            let alignmentInfo = try XCTUnwrap(alignments.randomElement())
    ...
    /// 5. 再次应用相同列表格式（切换）
    /// 6. 验证列表格式已移除
    func testProperty2_ListFormatApplicationCorrectness() throws {
        let iterations = 100
        ...
        for iteration in 1 ... iterations {
            ...
            // 3. 随机选择一个列表类型
            let listInfo = try XCTUnwrap(listTypes.randomElement())
    ...
    private func applyHeadingFormat(to range: NSRange, in textStorage: NSTextStorage, level _: HeadingLevel, size: CGFloat) {
