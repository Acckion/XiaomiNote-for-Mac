...
public final class SearchViewModel: ObservableObject {
    ...
    /// 过滤：私密笔记
    @Published public var filterIsPrivate: Bool = false
    ...
    private func applyFiltersToResults(_ results: [Note]) -> [Note] {
        ...
        if filterHasTags {
            filteredResults = filteredResults.filter { note in
                ...
                return !note.content.isEmpty // 简化实现
            }
        }
        ...
        if filterHasChecklist {
            filteredResults = filteredResults.filter { note in
                // 检查笔记是否包含清单
                return note.content.contains("☐") || note.content.contains("☑")
            }
        }
        ...
        if filterHasImages {
            filteredResults = filteredResults.filter { note in
                // 检查笔记是否包含图片
                return note.content.contains("<img") || note.content.contains("![")
            }
        }
        ...
        if filterHasAudio {
            filteredResults = filteredResults.filter { note in
                // 检查笔记是否包含音频
                return note.content.contains("<audio") || note.content.contains("🎵")
            }
        }
