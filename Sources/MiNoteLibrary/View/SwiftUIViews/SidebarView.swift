...
public struct SidebarView: View {
    ...
    private func selectAndRenameNewFolder(folderId: String, folderName: String) async {
        ...
        while attempts < maxAttempts {
            ...
            await MainActor.run {
                ...
                if let newFolder = nonSystemFolders.first(where: { $0.id == folderId }) {
                    ...
                    print("[SidebarView] ✅ 已设置 selectedFolder: '\(viewModel.selectedFolder?.name ?? "nil")' (ID: \(viewModel.selectedFolder?.id ?? "nil"))")
        ...
        await MainActor.run {
            ...
            print("[SidebarView] 所有文件夹列表: \(viewModel.folders.map { "\($0.id):'\($0.name)' (系统: \($0.isSystem), 创建于: \($0.createdAt))" }.joined(separator: ", "))")
        }
    }
