...
public struct XMLGenerator: Sendable {
    ...
    private func generateBlock(_ block: any BlockNode) -> String {
        ...
        case .titleBlock:
            ...
            guard let titleNode = block as? TitleBlockNode else {
                return "<text indent=\"1\">Error: Invalid title block</text>"
            }
            let content = generateInlineContent(titleNode.content)
            return "<text indent=\"1\">\(content)</text>"

        case .textBlock:
            guard let textNode = block as? TextBlockNode else {
                return "<text indent=\"1\">Error: Invalid text block</text>"
            }
            return generateTextBlock(textNode)

        case .bulletList:
            guard let bulletNode = block as? BulletListNode else {
                return "<text indent=\"1\">Error: Invalid bullet list</text>"
            }
        ...
        case .checkbox:
            guard let checkboxNode = block as? CheckboxNode else {
                return "<text indent=\"1\">Error: Invalid checkbox</text>"
            }
            return generateCheckbox(checkboxNode)

        case .horizontalRule:
            return generateHorizontalRule()

        case .image:
            guard let imageNode = block as? ImageNode else {
                return "<text indent=\"1\">Error: Invalid image</text>"
            }
            return generateImage(imageNode)

        case .audio:
            guard let audioNode = block as? AudioNode else {
                return "<text indent=\"1\">Error: Invalid audio</text>"
            }
            return generateAudio(audioNode)

        case .quote:
            guard let quoteNode = block as? QuoteNode else {
                return "<text indent=\"1\">Error: Invalid quote</text>"
            }
            return generateQuote(quoteNode)
...
public extension XMLGenerator {

    /// 从文档节点生成 XML 字符串（静态方法）
    /// - Parameter document: 文档 AST 节点
    /// - Returns: XML 字符串
    static func generate(_ document: DocumentNode) -> String {
