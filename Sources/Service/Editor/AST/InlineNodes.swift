...
public extension FormattedNode {
    /// 创建粗体节点
    static func bold(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .bold, content: content)
    }

    /// 创建粗体节点（单个文本）
    static func bold(_ text: String) -> FormattedNode {
        FormattedNode(type: .bold, content: [TextNode(text: text)])
    }

    /// 创建斜体节点
    static func italic(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .italic, content: content)
    }

    /// 创建斜体节点（单个文本）
    static func italic(_ text: String) -> FormattedNode {
        FormattedNode(type: .italic, content: [TextNode(text: text)])
    }

    /// 创建下划线节点
    static func underline(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .underline, content: content)
    }

    /// 创建下划线节点（单个文本）
    static func underline(_ text: String) -> FormattedNode {
        FormattedNode(type: .underline, content: [TextNode(text: text)])
    }

    /// 创建删除线节点
    static func strikethrough(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .strikethrough, content: content)
    }

    /// 创建删除线节点（单个文本）
    static func strikethrough(_ text: String) -> FormattedNode {
        FormattedNode(type: .strikethrough, content: [TextNode(text: text)])
    }

    /// 创建高亮节点
    static func highlight(_ content: [any InlineNode], color: String) -> FormattedNode {
        FormattedNode(type: .highlight, content: content, color: color)
    }

    /// 创建高亮节点（单个文本）
    static func highlight(_ text: String, color: String) -> FormattedNode {
        FormattedNode(type: .highlight, content: [TextNode(text: text)], color: color)
    }

    /// 创建大标题节点
    static func heading1(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .heading1, content: content)
    }

    /// 创建大标题节点（单个文本）
    static func heading1(_ text: String) -> FormattedNode {
        FormattedNode(type: .heading1, content: [TextNode(text: text)])
    }

    /// 创建二级标题节点
    static func heading2(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .heading2, content: content)
    }

    /// 创建二级标题节点（单个文本）
    static func heading2(_ text: String) -> FormattedNode {
        FormattedNode(type: .heading2, content: [TextNode(text: text)])
    }

    /// 创建三级标题节点
    static func heading3(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .heading3, content: content)
    }

    /// 创建三级标题节点（单个文本）
    static func heading3(_ text: String) -> FormattedNode {
        FormattedNode(type: .heading3, content: [TextNode(text: text)])
    }

    /// 创建居中对齐节点
    static func centerAlign(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .centerAlign, content: content)
    }

    /// 创建居中对齐节点（单个文本）
    static func centerAlign(_ text: String) -> FormattedNode {
        FormattedNode(type: .centerAlign, content: [TextNode(text: text)])
    }

    /// 创建右对齐节点
    static func rightAlign(_ content: [any InlineNode]) -> FormattedNode {
        FormattedNode(type: .rightAlign, content: content)
    }

    /// 创建右对齐节点（单个文本）
    static func rightAlign(_ text: String) -> FormattedNode {
