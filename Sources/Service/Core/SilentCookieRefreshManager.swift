...
public final class SilentCookieRefreshManager: NSObject {
    ...
    /// 响应式刷新会忽略冷却期限制，立即执行
    private var isReactiveRefresh = false
    ...
    public func isInCooldownPeriod() -> Bool {
        ...
        if inCooldown {
            print(
                "[SilentCookieRefreshManager] 在冷却期内，已过 \(String(format: "%.1f", elapsed)) 秒，需等待 \(String(format: "%.1f", cooldownPeriod - elapsed)) 秒"
            )
        }
        return inCooldown
    }
    ...
    private func synchronizeCookiesAndVerify(cookies: [HTTPCookie]) -> Bool {
        ...
        if isConsistent {
            ...
            print(
                "[SilentCookieRefreshManager] ❌ Cookie 同步失败: WKWebView serviceToken=\(webViewServiceToken.prefix(20))..., HTTPStorage serviceToken=\(httpStorageServiceToken.prefix(20))..."
            )
        }
    ...
    private func extractCookieFromWebView() {
        guard let webView, !cookieExtracted else { return }
        ...
        webView.configuration.websiteDataStore.httpCookieStore.getAllCookies { (cookies: [HTTPCookie]) in
            Task { @MainActor in
                ...
                if hasServiceToken, hasUserId, !cookieString.isEmpty {
                    ...
                    print(
                        "[SilentCookieRefreshManager] ⚠️ Cookie 验证失败: hasServiceToken=\(hasServiceToken), hasUserId=\(hasUserId), cookieString长度=\(cookieString.count)"
                    )
                    // 继续等待或重试
                }
            }
