...
final class NetworkRequestManager: ObservableObject {
            ...
            processingQueue.async { [weak self] in
                ...
                Task {
                    do {
                        ...
                        // 缓存响应
                        if case .cache(let ttl) = request.cachePolicy {
                            Task { @MainActor in
    ...
    private func addToQueue(_ request: NetworkRequest) {
        queue.async(flags: .barrier) { [weak self] in
            ...
            // 按优先级插入
            var inserted = false
            for (index, existingRequest) in self.pendingRequests.enumerated() {
                if request.priority > existingRequest.priority {
                    self.pendingRequests.insert(request, at: index)
                    inserted = true
                    break
                }
            }
    ...
    /// 处理HTTP错误
    private nonisolated func handleHTTPError(statusCode: Int, request: NetworkRequest) -> Error {
        switch statusCode {
        case 401:
            return MiNoteError.cookieExpired
        case 403:
            return MiNoteError.notAuthenticated
        case 404:
            return NSError(domain: "NetworkRequestManager", code: 404, userInfo: [NSLocalizedDescriptionKey: "资源不存在"])
        case 429:
            return NSError(domain: "NetworkRequestManager", code: 429, userInfo: [NSLocalizedDescriptionKey: "请求过于频繁"])
        case 500...599:
            return MiNoteError.networkError(NSError(domain: "NetworkRequestManager", code: statusCode, userInfo: [NSLocalizedDescriptionKey: "服务器错误"]))
        default:
            return MiNoteError.networkError(NSError(domain: "NetworkRequestManager", code: statusCode, userInfo: [NSLocalizedDescriptionKey: "HTTP错误: \(statusCode)"]))
        }
    }
}
