...
public final class NativeFormatProvider: FormatMenuProvider {
    ...
    /// 性能监控是否启用
    private var performanceMonitoringEnabled: Bool = true
    
    /// 性能记录
    private var performanceRecords: [FormatDetectionPerformanceRecord] = []
    
    /// 最大记录数量
    private let maxRecordCount: Int = 200
    ...
    /// 最小检测时间（毫秒）
    private var minDetectionTime: Double = Double.infinity
    ...
    ///   - range: 选择范围
    /// _Requirements: 5.1, 5.2, 5.3_
    private func applyFormatToSelection(_ format: TextFormat, range: NSRange) {
...
extension NativeFormatProvider {
...
public enum FormatDetectionError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .editorNotAvailable:
            return "编辑器不可用"
        case .invalidRange(let location, let length, let textLength):
            return "无效的选择范围: location=\(location), length=\(length), textLength=\(textLength)"
        case .emptyDocument:
            return "文档为空"
        case .cursorOutOfBounds(let position, let textLength):
            return "光标位置超出范围: position=\(position), textLength=\(textLength)"
