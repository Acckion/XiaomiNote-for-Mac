...
final class EditorStateConsistencyChecker: ObservableObject {
    ...
    /// 格式按钮是否应该启用
    @Published private(set) var formatButtonsEnabled = false

    /// 状态变化原因
    @Published private(set) var stateChangeReason = ""
    ...
    /// 连续不一致计数
    private var inconsistencyCount = 0

    /// 不一致阈值
    private let inconsistencyThreshold = 3
    ...
    func validateFormatOperation(_ format: TextFormat) -> Bool {
        guard currentState.allowsFormatting else {
            // 记录错误
            let error: FormatError = switch currentState {
            case .readOnly:
                .editorNotEditable
            case .unfocused:
                .editorNotFocused
            ...
            default:
                .formatApplicationFailed(format: format.displayName, reason: currentState.description)
            }
