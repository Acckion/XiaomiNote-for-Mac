...
final class FormatMenuDebugger: ObservableObject {
    ...
    /// 实时统计数据
    @Published var statistics = FormatMenuStatistics()
    ...
    private func updateStatistics() {
        ...
        let successfulApplications = applicationRecords.count(where: { $0.success })
        ...
        let avgApplicationTime = applicationRecords.isEmpty ? 0 :
            applicationRecords.map(\.duration).reduce(0, +) / Double(applicationRecords.count)

        let avgSynchronizationTime = synchronizationRecords.isEmpty ? 0 :
            synchronizationRecords.map(\.duration).reduce(0, +) / Double(synchronizationRecords.count)

        // 计算超过阈值的次数
        let slowApplications = applicationRecords.count(where: { $0.duration > formatApplicationThreshold })
        let slowSynchronizations = synchronizationRecords.count(where: { $0.duration > stateSynchronizationThreshold })
...
struct FormatMenuStatistics {
    ...
    var failedApplications = 0
    var totalSynchronizations = 0
    var avgApplicationTimeMs: Double = 0
    var avgSynchronizationTimeMs: Double = 0
    var slowApplications = 0
    var slowSynchronizations = 0
