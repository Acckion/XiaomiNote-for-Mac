...
final class FormatErrorHandler {
    ...
    func handleError(_ error: FormatError, context: FormatErrorContext = .empty) -> FormatErrorHandlingResult {
        ...
        // 4. 根据错误类型处理
        let result: FormatErrorHandlingResult = if consecutiveErrorCount[errorCode]! >= consecutiveErrorThreshold {
            // 连续错误过多，触发特殊处理
            handleRepeatedError(error, context: context)
        } else if enableAutoRecovery, error.isRecoverable {
            performAutoRecovery(for: error, context: context)
        } else {
            FormatErrorHandlingResult.unhandled(
                error: error,
    ...
    func handleRangeError(range: NSRange, textLength: Int) -> FormatErrorHandlingResult {
        let error: FormatError = if range.location + range.length > textLength {
            .rangeOutOfBounds(range: range, textLength: textLength)
        } else {
            .invalidRange(range: range, textLength: textLength)
        }
    ...
    /// 处理重复错误
    private func handleRepeatedError(_ error: FormatError, context _: FormatErrorContext) -> FormatErrorHandlingResult {
    ...
    func getErrorStatistics() -> [String: Any] {
        ...
        let handledErrors = errorHistory.count(where: { $0.handled })
        ...
        // 按恢复操作分组统计
        let recoveryActionCounts = Dictionary(grouping: errorHistory.compactMap(\.recoveryAction)) { $0 }
            .mapValues { $0.count }
