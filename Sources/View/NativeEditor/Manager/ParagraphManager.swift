...
public class ParagraphManager {
    ...
    internal func setParagraphs(_ paragraphs: [Paragraph]) {
    ...
    ///   - textStorage: 文本存储
    ///   - changedRange: 变化的范围
    public func updateParagraphs(in textStorage: NSTextStorage, changedRange: NSRange) {
        // 1. 检测新的段落边界
        ...
        for (index, newRange) in newParagraphRanges.enumerated() {
            // 尝试找到对应的旧段落
            // 策略：查找起始位置最接近的旧段落
            var matchedOldParagraph: Paragraph? = nil
    ...
    private func applyHeadingFormat(level: Int, to range: NSRange, in textStorage: NSTextStorage) {
        ...
        switch level {
        case 1: fontSize = 23  // H1
        case 2: fontSize = 20  // H2
        case 3: fontSize = 17  // H3
        case 4: fontSize = 16  // H4
        case 5: fontSize = 15  // H5
        case 6: fontSize = 14  // H6
        default: fontSize = 14
        }
