...
public class UndoCoalescingManager {
    ...
    /// 当前分组是否处于活动状态
    private var isGroupActive: Bool = false
    ...
    public func shouldStartNewGroup(for change: String, at location: Int) -> Bool {
        ...
        case .hybrid(let strategies):
            ...
            for subStrategy in strategies {
                ...
                tempManager.currentGroupStart = self.currentGroupStart
                tempManager.lastInputTime = self.lastInputTime
                tempManager.lastInputLocation = self.lastInputLocation
                tempManager.isGroupActive = self.isGroupActive
    ...
    ///   - change: 文本变化内容
    ///   - location: 变化位置
    public func recordInput(change: String, at location: Int) {
        lastInputTime = Date()
        lastInputLocation = location
...
extension UndoCoalescingManager {
    /// 创建默认的混合策略
    /// 
    ...
    /// - Returns: 混合策略
    public static func defaultHybridStrategy(
    ...
    /// 
    /// - Returns: 代码编辑策略
    public static func codeEditingStrategy() -> UndoCoalescingStrategy {
        return .hybrid([
            .wordBased,
            .timeBased(1.5)
        ])
    ...
    /// 
    /// - Returns: 长文本编辑策略
    public static func longTextEditingStrategy() -> UndoCoalescingStrategy {
        return .hybrid([
            .continuous,
            .timeBased(3.0)
        ])
