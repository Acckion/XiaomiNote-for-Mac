...
final class NativeEditorInitializer {
    ...
    func isNativeEditorSupported() -> Bool {
        return checkSystemCompatibility().isCompatible
    }
    ...
    /// 验证编辑器上下文
    private func validateEditorContext(_ context: NativeEditorContext) throws {
        // 验证格式转换器
        let testXML = "<text indent=\"1\">测试</text>"
        do {
            let _ = try XiaoMiFormatConverter.shared.xmlToAttributedString(testXML)
        } catch {
        ...
        let _ = renderer.createCheckboxAttachment(checked: false, level: 3, indent: 1)
        let _ = renderer.createHorizontalRuleAttachment()
        let _ = renderer.createBulletAttachment(indent: 1)
        let _ = renderer.createOrderAttachment(number: 1, indent: 1)
