...
struct CacheConfiguration {
    /// 附件缓存最大数量
    var maxAttachmentCacheSize: Int = 200
    
    /// 图像缓存最大数量
    var maxImageCacheSize: Int = 50
    
    /// 图像缓存最大内存（字节）
    var maxImageCacheMemory: Int64 = 100 * 1024 * 1024 // 100MB
    
    /// 渲染结果缓存最大数量
    var maxRenderCacheSize: Int = 100
    
    /// 缓存过期时间（秒）
    var cacheExpirationTime: TimeInterval = 300 // 5分钟
    
    /// 是否启用增量渲染
    var enableIncrementalRendering: Bool = true
    
    /// 增量渲染阈值（行数）
    var incrementalRenderingThreshold: Int = 100
...
struct CacheEntry<T> {
    ...
    func isExpired(expirationTime: TimeInterval) -> Bool {
        return Date().timeIntervalSince(lastAccessedAt) > expirationTime
    }
}
...
class LRUCache<Key: Hashable, Value> {
    ...
    func clearExpired() {
        ...
        let expiredKeys = cache.filter { $0.value.isExpired(expirationTime: expirationTime) }.map { $0.key }
...
class PerformanceOptimizer {
    ...
    func getPerformanceReport() -> String {
        ...
        if !metricsHistory.isEmpty {
            let avgInitTime = metricsHistory.map { $0.initializationTime }.reduce(0, +) / Double(metricsHistory.count)
            let avgRenderTime = metricsHistory.map { $0.renderTime }.reduce(0, +) / Double(metricsHistory.count)
    ...
    func getCacheStats() -> (attachments: Int, images: Int, renders: Int) {
        return (attachmentCache.count, imageCache.count, renderCache.count)
    }
    ...
    func shouldUseIncrementalRendering(lineCount: Int) -> Bool {
        return configuration.enableIncrementalRendering &&
