...
struct NotesListViewControllerWrapper: NSViewControllerRepresentable {
    ...
    func makeNSViewController(context: Context) -> NotesListViewController {
        let controller = NotesListViewController(viewModel: viewModel)
        return controller
    }
    
    func updateNSViewController(_ nsViewController: NotesListViewController, context: Context) {
        // 视图模型更新时，不需要额外操作
