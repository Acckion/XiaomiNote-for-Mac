...
struct NoteDetailView: View {
    ...
    private func loadNoteContentFromCache(_ note: Note) async {
        ...
        guard note.id == currentEditingNoteId else {
            Swift
                .print(
                    "[快速切换] ⚠️ loadNoteContentFromCache: 笔记ID不匹配，取消加载 - 传入ID: \(note.id.prefix(8))..., 当前编辑ID: \(currentEditingNoteId?.prefix(8) ?? "nil")"
                )
            return
        }
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            DatabaseService.shared.saveNoteAsync(updated) { error in
                Task { @MainActor in
                    ...
                    Swift.print("[保存流程] 📝 标题和内容保存成功，lastSavedXMLContent 已同步 - 长度: \(lastSavedXMLContent.count)")
                    // 更新笔记列表和选中的笔记
                    if let index = viewModel.notes.firstIndex(where: { $0.id == updated.id }) {
                        viewModel.notes[index] = updated
                    }
    ...
    private func updateMemoryCache(xmlContent: String, htmlContent _: String?, for note: Note) async {
        ...
        guard note.id == currentEditingNoteId else {
            Swift
                .print(
                    "[保存流程] ⚠️ updateMemoryCache: 笔记ID不匹配，忽略更新 - 传入ID: \(note.id.prefix(8))..., 当前编辑ID: \(currentEditingNoteId?.prefix(8) ?? "nil")"
                )
            return
        }
    ...
    private func scheduleXMLSave(xmlContent: String, for note: Note, extractedTitle: TitleExtractionResult? = nil, immediate: Bool = false) {
        ...
        guard note.id == currentEditingNoteId else {
            Swift
                .print("[保存流程] ⏭️ Tier 1 跳过 - 不是当前编辑笔记，ID: \(note.id.prefix(8))..., currentEditingNoteId: \(currentEditingNoteId?.prefix(8) ?? "nil")")
            return
        }
    ...
    private func performXMLSave(xmlContent: String, for note: Note, extractedTitle _: TitleExtractionResult? = nil) {
        ...
        xmlSaveTask = Task { @MainActor in
            ...
            do {
                ...
                Swift
                    .print(
                        "[保存流程] ✅ SavePipelineCoordinator 保存成功 - 标题: '\(result.extractedTitle)', 耗时: \(String(format: "%.2f", result.executionTime))秒"
                    )
            } catch {
                // 检查任务是否被取消或笔记已切换
                guard !Task.isCancelled, currentEditingNoteId == noteId else {
    ...
    private func handleSaveSuccess(xmlContent: String, noteId: String, updatedNote: Note) async {
        ...
        lastSavedXMLContent = xmlContent
        currentXMLContent = xmlContent
        Swift.print("[保存流程] 📝 XML保存成功，lastSavedXMLContent 已同步 - 长度: \(lastSavedXMLContent.count)")
        ...
        // 更新视图模型中的笔记
        let oldSelectedNoteId = viewModel.selectedNote?.id
        ...
        let newSelectedNoteId = viewModel.selectedNote?.id
        ...
        Swift.print("[保存流程] ✅ Tier 1 本地保存成功 - 笔记ID: \(noteId.prefix(8))..., 标题: \(editedTitle)")
    }
        ...
        if contentChanged {
            ...
            if normalizedLengthDiff <= 50 {
                ...
                if let diffIndex = firstDiffIndex {
                    ...
                    let currentContext =
                        String(normalizedCurrent[normalizedCurrent.index(normalizedCurrent.startIndex, offsetBy: contextStart) ..< normalizedCurrent
                                .index(
                                    normalizedCurrent.startIndex,
                                    offsetBy: contextEnd
                                )])
                    let savedContext =
                        String(normalizedSaved[normalizedSaved.index(normalizedSaved.startIndex, offsetBy: contextStart) ..< normalizedSaved.index(
                            normalizedSaved.startIndex,
                            offsetBy: contextEnd
                        )])
