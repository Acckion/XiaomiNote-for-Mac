...
struct CookieRefreshView: View {
    ...
    @State private var isLoading = true // 初始状态为加载中，因为页面需要加载
    @State private var showError = false
    @State private var errorMessage = ""
    @State private var isCookieRefreshed = false
...
struct CookieRefreshWebView: NSViewRepresentable {
    ...
    var autoClickLoginButton = false
        ...
        func webView(_ webView: WKWebView, didFinish _: WKNavigation!) {
            ...
            if currentURL.contains("i.mi.com/status/lite/profile") {
                ...
                webView.configuration.websiteDataStore.httpCookieStore.getAllCookies { [weak self] cookies in
                    guard let self, !self.cookieExtracted else { return }
                    ...
                    if let profileURL = URL(string: "https://i.mi.com/status/lite/profile?ts=\(Int(Date().timeIntervalSince1970 * 1000))") {
                        ...
                        let task = session.dataTask(with: request) { [weak self] _, _, _ in
                            ...
                            if hasServiceToken, hasUserId, !cookieString.isEmpty {
                                ...
                                print(
                                    "[CookieRefreshWebView] ⚠️ Cookie 验证失败: hasServiceToken=\(hasServiceToken), hasUserId=\(hasUserId), cookieString长度=\(cookieString.count)"
                                )
                    ...
                    if hasServiceToken, hasUserId, !cookieString.isEmpty {
                        ...
                        DispatchQueue.main.async {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                if let profileURL =
                                    URL(string: "https://i.mi.com/status/lite/profile?ts=\(Int(Date().timeIntervalSince1970 * 1000))")
                                {
                        ...
                        if parent.autoClickLoginButton {
