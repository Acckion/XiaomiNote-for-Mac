...
enum EditorCreationError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .unsupportedType(type):
            "不支持的编辑器类型: \(type.displayName)"
        case let .systemRequirementsNotMet(type, requirement):
            "\(type.displayName) 不满足系统要求: \(requirement)"
        case let .initializationFailed(type, error):
            "\(type.displayName) 初始化失败: \(error.localizedDescription)"
        }
    }
}
...
class EditorFactory {
    ...
    /// 编辑器创建缓存
    private nonisolated(unsafe) static var editorCache: [EditorType: () -> EditorProtocol] = [:]
    ...
    static func createEditorSafely(type: EditorType) throws -> EditorProtocol {
        try createEditor(type: type)
    }

    /// 检查编辑器类型是否可用
    /// - Parameter type: 编辑器类型
    /// - Returns: 是否可用
    static func isEditorAvailable(_: EditorType) -> Bool {
    ...
    static func getAvailableEditorTypes() -> [EditorType] {
        supportedTypes.filter { isEditorAvailable($0) }
    }
    ...
    private static func createNativeEditor() throws -> EditorProtocol {
        ...
        throw EditorCreationError.initializationFailed(
            .native,
            NSError(domain: "EditorFactory", code: -1, userInfo: [NSLocalizedDescriptionKey: "请使用 NativeEditorContext 创建原生编辑器"])
        )
    }
