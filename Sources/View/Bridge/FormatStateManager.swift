...
public final class FormatStateManager: ObservableObject {
    ...
    /// 当前格式状态
    /// _Requirements: 8.1_
    @Published public private(set) var currentState: FormatState = FormatState()
    
    /// 当前活动的格式提供者
    @Published public private(set) var activeProvider: (any FormatMenuProvider)?
    
    /// 是否有活动的编辑器
    @Published public private(set) var hasActiveEditor: Bool = false
    ...
    /// 跳过的更新次数（因为状态未变化）
    private var skippedUpdateCount: Int = 0
    ...
    public func isFormatActive(_ format: TextFormat) -> Bool {
        return currentState.isFormatActive(format)
    }
    
    /// 获取当前段落格式
    /// - Returns: 当前段落格式
    public func getCurrentParagraphFormat() -> ParagraphFormat {
        return currentState.paragraphFormat
    }
    
    /// 获取当前对齐格式
    /// - Returns: 当前对齐格式
    public func getCurrentAlignment() -> AlignmentFormat {
        return currentState.alignment
    }
