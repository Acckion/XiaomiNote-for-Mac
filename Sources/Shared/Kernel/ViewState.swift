...
public struct ViewState: Equatable, Codable {
    ...
    ///   - folders: 文件夹列表
    /// - Returns: 状态是否一致
    public func isConsistent(with notes: [Note], folders _: [Folder]) -> Bool {
        // 如果没有选中笔记，状态一致
...
public enum StateInconsistency: Equatable {
    ...
    public var description: String {
        switch self {
        case let .noteNotInFolder(noteId, folderId):
            "笔记 \(noteId) 不属于文件夹 \(folderId)"
        case let .noteNotFound(noteId):
            "笔记 \(noteId) 不存在"
        case let .folderNotFound(folderId):
            "文件夹 \(folderId) 不存在"
        }
    }
}
